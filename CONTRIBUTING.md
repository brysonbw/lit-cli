...
We welcome and appreciate all contributions from the community. By contributing to `lit-cli`, you agree to abide by the [code of conduct](/CODE_OF_CONDUCT.md).
...
> [!NOTE]
> You can also open the documentation locally by running `cargo doc --open`.

### Code

Unsure of where to begin contributing to `lit-cli`? You can start by looking through the issues labeled _good-first-issue_ and _help-wanted_. You can also start by contributing to the project documentation (e.g files with extension `.md`).
...
1. [Fork](https://docs.github.com/en/github/getting-started-with-github/fork-a-repo) the `lit-cli` repository.
...
6. Build:
   ...
   cargo build
   ```
7. Open the `lit-cli` folder in your favorite editor. If you don't have one, try [Visual Studio Code](https://code.visualstudio.com/)
...
Once you have cloned `lit-cli` and completed [Setup](#setup), you can lint, format, build, and test the code from your terminal.
...
To compile the `lit-cli` source, run:
...
Once you have finished working on an issue or feature, you can submit a pull request to have your changes merged into the `lit-cli` repository and included in the next release.
